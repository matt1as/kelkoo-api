...
export class LinksApi {
  ...
  /**
   *
   * @summary Monetize a merchant URL by getting an offer or a merchant directory link
   * @param country Country
   * @param merchantUrl Merchant URL
   * @param fallback Enable merchant match. If false, only offer match is enabled.
   * @param custom1 Custom parameter 1
   * @param custom2 Custom parameter 2
   * @param custom3 Custom parameter 3
   */
  public async searchOfferLink(
    country:
      | 'ae'
      | 'at'
      | 'au'
  ...
  ): Promise<{ response: http.IncomingMessage; body: LinkResponse }> {
    const localVarPath = this.basePath + '/search/link';
    ...
    if (country === null || country === undefined) {
      throw new Error('Required parameter country was null or undefined when calling searchOfferLink.');
    }

    // verify required parameter 'merchantUrl' is not null or undefined
    if (merchantUrl === null || merchantUrl === undefined) {
      throw new Error('Required parameter merchantUrl was null or undefined when calling searchOfferLink.');
    }
    ...
    if (merchantUrl !== undefined) {
      localVarQueryParameters['merchantUrl'] = ObjectSerializer.serialize(merchantUrl, 'string');
    }

    if (fallback !== undefined) {
      localVarQueryParameters['fallback'] = ObjectSerializer.serialize(fallback, 'boolean');
    }
